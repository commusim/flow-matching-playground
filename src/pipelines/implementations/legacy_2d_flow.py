...
def scatter(ax, points, color, label=None, alpha=0.65, size=7):
    ...
    ax.scatter(p[:, 0], p[:, 1], s=size, c=color, alpha=alpha,
...
def plot_overview(model, device, output_path, particles):
    ...
    for t_value, color in zip([0.0, 0.25, 0.5, 0.75, 1.0],
                              ["#94A3B8", "#60A5FA", "#A78BFA", "#FB7185", "#EF4444"]):
        xt = (1.0 - t_value) * x_noise + t_value * x_data
        scatter(axes[0, 2], xt[:350], color, label="t={:.2f}".format(t_value), alpha=0.42, size=6)
    ...
    axes[1, 0].quiver(qx[:, 0], qx[:, 1], qv[:, 0], qv[:, 1],
                      color="#7C3AED", alpha=0.70, angles="xy", scale_units="xy", scale=8)
    style_axis(axes[1, 0], "4. Training targets at t=0.5\n(pairwise conditional velocities)")

    gx, gy = np.meshgrid(np.linspace(-2.7, 2.7, 18), np.linspace(-2.3, 2.3, 16))
    grid = torch.tensor(np.stack([gx.ravel(), gy.ravel()], axis=1), dtype=torch.float32, device=device)
    tgrid = torch.full((len(grid), 1), 0.5, device=device)
    learned = model(grid, tgrid).cpu().numpy()
    speed = np.linalg.norm(learned, axis=1)
    axes[1, 1].quiver(grid[:, 0].cpu(), grid[:, 1].cpu(), learned[:, 0], learned[:, 1], speed,
                      cmap="viridis", alpha=0.9, angles="xy", scale_units="xy", scale=7)
    ...
    scatter(axes[1, 2], trajectory[-1], COLORS["generated"], label="generated", alpha=0.62, size=7)
    style_axis(axes[1, 2], "6. ODE result at t=1")
    axes[1, 2].legend(frameon=False, fontsize=9, markerscale=2)

    fig.suptitle("2D Flow Matching: learn a time-dependent velocity field", fontsize=17, fontweight="bold")
...
def plot_trajectory(trajectory, target, output_path):
    ...
    fig.suptitle("ODE sampling: particles follow the learned vector field", fontsize=16, fontweight="bold")
    fig.text(0.5, 0.005, "Red shadow = target distribution    Blue = transported particles",
...
def plot_velocity_fields_over_time(model, device, trajectory, output_path):
    ...
    for ax, t_value in zip(axes.ravel(), times):
        ...
        ax.scatter(particles[:, 0], particles[:, 1], s=5, c="#CBD5E1",
                   alpha=0.28, edgecolors="none", rasterized=True)
        q = ax.quiver(grid_np[:, 0], grid_np[:, 1], velocity[:, 0], velocity[:, 1], speed,
                      cmap="viridis", clim=(0.0, 3.0), angles="xy",
                      scale_units="xy", scale=8.0, alpha=0.92, width=0.004)
        quivers.append(q)
    ...
    fig.suptitle("The learned velocity field changes with time",
...
def plot_loss(losses, output_path):
    ...
    ax.plot(np.arange(window - 1, len(values)), smooth, color="#1D4ED8", linewidth=2, label="moving average")
...
def make_animation(trajectory, target, output_path):
    ...
    ax.scatter(target_np[:, 0], target_np[:, 1], s=8, c=COLORS["data"], alpha=0.12, edgecolors="none")
    current = ax.scatter([], [], s=10, c=COLORS["generated"], alpha=0.72, edgecolors="none")
    title = ax.set_title("")
    style_axis(ax, "")
    ax.text(0.02, 0.02, "red shadow: target", transform=ax.transAxes, fontsize=9, color="#B91C1C")

    def update(frame_number):
        idx = keep[frame_number]
        pts = trajectory[idx].numpy()
        current.set_offsets(pts)
        title.set_text("Flow Matching ODE sampling   t={:.2f}".format(idx / float(len(trajectory) - 1)))
...
def main():
    ...
    losses = train(model, device, args.steps, args.batch_size, args.lr,
    ...
    plot_overview(model, device, os.path.join(args.output_dir, "flow_matching_overview.png"), args.particles)
    plot_trajectory(trajectory, target, os.path.join(args.output_dir, "sampling_trajectory.png"))
    plot_velocity_fields_over_time(model, device, trajectory, os.path.join(args.output_dir, "velocity_fields_over_time.png"))
    if not args.no_animation:
        make_animation(trajectory, target, os.path.join(args.output_dir, "flow_animation.gif"))

    print("\nDone. Open the files in '{}' to inspect the learned flow.".format(args.output_dir))
