# 项目Checklist

## 基础结构

- [x] `src/utils/` 已建立，承载公共基础能力与工具
- [x] `src/modules/` 已建立
- [x] `src/pipelines/` 已建立
- [x] `src/cli/` 已建立
- [x] 根目录独立存在 `configs/`、`data/`、`outputs/`、`main.py`
- [x] 统一入口支持 `--pipeline`
- [x] 所有旧实验已完成内部模块化拆分

## Flow Matching原理

- [x] 无条件二维速度场拟合
- [x] 条件二维速度场
- [x] 多时间速度场可视化
- [x] ODE采样轨迹可视化
- [x] 同噪声不同条件轨迹对比
- [x] 条件速度差分场
- [x] Sinkhorn最优传输耦合
- [x] OT运输距离和路径可视化

## 简单图像扩展

- [x] MNIST数据加载pipeline
- [ ] Fashion-MNIST或CIFAR-10子集
- [x] MNIST监督识别模型与语义特征t-SNE
- [x] VAE latent条件Flow
- [x] 多尺度AdaGN条件U-Net Flow
- [x] U-Net合并到image_velocity统一模块
- [x] 图像尺寸、通道数和类别数Config化
- [x] MNIST/Fashion-MNIST/CIFAR-10/ImageFolder通用加载
- [x] 识别模型统一评价生成准确率
- [x] 统一分类器语义轨迹与单一联合t-SNE
- [x] Label中心语义轨迹、准确率与置信度曲线
- [x] 真实类别簇参照与初始目标Label着色
- [x] 分类器终点判定、置信度阈值与unknown类别
- [x] 每个目标Label 10个样本的正式轨迹对比
- [x] 分类器128维语义速度场与线性PCA矢量投影
- [x] 语义速度、目标簇距离和目标方向对齐曲线
- [x] 简单图像的像素空间Flow Matching
- [x] 小型VAE编码/解码实验
- [x] 图像latent轨迹可视化
- [x] Label条件MNIST生成
- [x] 逐层AdaGN/FiLM条件残差块
- [x] Label速度分离度诊断指标
- [x] 空间金字塔特征PCA轨迹
- [x] 空间特征PCA图与激活热力图
- [x] MNIST checkpoint加载
- [x] 三种MNIST模型版本与旧checkpoint自动识别

## 科学图像研究准备

- [ ] VAE重建前后SIQA对比
- [ ] MMDiT与Decoder模块瓶颈实验
- [ ] 正确/错误Prompt最小差异实验
- [ ] 正确与错误语义的latent可分性实验
- [ ] SIU2A评估接入
- [ ] 科学图像生成pipeline

## 文档与展示

- [x] README已更新
- [x] 经验理论文档已建立
- [x] 交互式HTML教程已建立
- [x] 五章节GitHub Pages教学站点
- [x] MathJax公式与LaTeX数学说明
- [x] 二维无条件/条件章节分离
- [x] MNIST四模型横向时间演进
- [x] 教程时间点扩展到41步
- [x] 二维时间/条件/速度场交互
- [x] MNIST模型/Label/时间图像演进交互
- [x] 共享t-SNE流形与PCA语义速度场交互
- [x] 每Label 10个独立噪声逐点41步轨迹
- [x] PCA/UMAP/t-SNE三种语义流形切换
- [x] 三种降维方式的速度/嵌入位移切换
- [x] 每Label四噪声0000–9999条件矩阵
- [x] 加粗PCA语义速度箭头与增强真实簇背景
- [x] 四模型×十Label终态条件生成矩阵
- [x] 教程资产可复现生成脚本
- [ ] GitHub Pages自动部署
- [ ] 每个pipeline生成独立图像说明文档
