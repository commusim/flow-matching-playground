...
共享t-SNE交互现在对所选目标Label使用10个不同初始噪声，并分别绘制每个样本的41步轨迹；真实参照背景增加到每类150个样本。t-SNE折线只表示邻域投影，可能因非线性降维出现视觉跳变。

语义速度场不再使用三角形，改为从当前类别中心直接绘制加粗箭头，并按固定视觉时间尺度放大；真实类别簇背景也提高了不透明度。MNIST时间演进之前新增“四模型×十Label”的最终条件生成矩阵。

### PCA / UMAP / t-SNE三种语义流形

语义流形交互支持三种降维方式实时切换：PCA使用统一线性基底，适合连续轨迹与方向解释；UMAP强调非线性类别流形和邻域；t-SNE强调局部类别簇。三者使用相同的1500个真实参考样本、每Label 10个噪声和41个时间点。

语义速度面板同样支持三种方式：PCA箭头为128维语义速度的真实线性投影；UMAP和t-SNE箭头为相邻时间点在非线性嵌入中的有限位移，不能当作严格速度向量。

最终条件生成面板改为模型可切换；每个Label展示4个不同初始噪声，因此依次看到`0000、1111、…、9999`，用于区分条件控制与噪声多样性。
